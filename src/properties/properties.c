...
PetscReal SaltWaterMassDiffusivity(PetscReal temperature, PetscReal salinity)
{
    PetscReal a[3] = {1.47294,
                      0.12642,
                      0.01079};

    PetscReal density, molarity, density_NaCl, molar_mass_NaCl, diffusivity;
    ...
    diffusivity = a[0];
    diffusivity += a[1] * PetscExpReal(-26.3969 * molarity);
    diffusivity += a[2] * PetscPowReal(molarity, 1.93559);
    diffusivity *= 1.0e-9;
...
PetscErrorCode SaltWaterPropBuild(SaltWaterProperties *salt_water_prop, PetscReal temperature, PetscReal salinity)
{
    ...
    mass_diffusivity = SaltWaterMassDiffusivity(temperature, salinity);
    ...
    salt_water_prop->schmidt = dyn_viscosity / (density * mass_diffusivity);
    salt_water_prop->vapor_pressure = vapor_pressure;
    salt_water_prop->latent_heat_vaporization = latent_heat_vaporization;
    salt_water_prop->mass_diffusivity= mass_diffusivity;
